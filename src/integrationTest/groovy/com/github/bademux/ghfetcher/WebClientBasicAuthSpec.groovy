...
@Narrative("""
Here we test scenario where 1st call fetches an entity and 2nd call gets entity from local cache
""")
...
@TestPropertySource(properties = [
        ...
        'githubClient.auth.basic.name=bademux',
        'githubClient.auth.basic.password=XXX'
])
class WebClientBasicAuthSpec extends Specification {

    final static WireMockClassRule wireMock = new WireMockClassRule(wireMockConfig()
            .extensions(new DateHeaderResponseTransformer(Instant.&now))
            .withRootDirectory("src/integrationTest/resources/${WebClientBasicAuthSpec.simpleName}")
            .dynamicPort())
